...
PetscErrorCode updateDanglingNodeDisp(Box *box_ptr, Node *alph_ptr, Node *beta_ptr, Node *delt_ptr)
{
    ...
    ierr = makeDistanceVec(s_betaDelt, s_beta, s_delt, box_ptr->xyzPeriodic, box_ptr->xyzDimension);CHKERRQ(ierr);
    l_betaDelt = vecMagnitude(s_betaDelt);

    /* use new distance vector to find new orientation of dangling segment */
    ierr = makeDistanceVec(s_nalpBeta, s_nalp, s_nbet, box_ptr->xyzPeriodic, box_ptr->xyzDimension);CHKERRQ(ierr);
